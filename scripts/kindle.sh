...
_send() {
    ...
    # Send mail
    neomutt -s $subject -a $attachment -- $mail < /dev/null
    if [[ $? -ne 0 ]]; then
...
command -v neomutt >/dev/null 2>&1 || { echo >&2 "Mutt is required but it was not found. Please install it"; exit 1; }
